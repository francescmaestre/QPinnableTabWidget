        ...
        void mouseMoveEvent(QMouseEvent* event) override
        {
            ...
            if (pinnableWidget)
            {
                ...
                if (!currentPinned && !newPosIsPinned
                    && (event->pos().x() - mDistToStart) > tabRect(lastPinnedTab).right())
...
int QPinnableTabWidget::addTab(QWidget* widget, const QString& s) { return QTabWidget::addTab(widget, s); }
...
void QPinnableTabWidget::changeTabName(int index, const QString& s) { QTabWidget::setTabText(index, s); }
...
bool QPinnableTabWidget::isPinned(int index) { return mTabState.contains(index); }

int QPinnableTabWidget::getLastPinnedTabIndex() const { return mLastPinTab - 1; }
...
void QPinnableTabWidget::showContextMenu()
{
    ...
    connect(actions->addAction("Close"), &QAction::triggered, this, [this]() {
...
void QPinnableTabWidget::unpinTab()
{
    ...
    connect(closeBtn, &RealCloseButton::clicked, this, [this]() {
        emit tabBar()->tabCloseRequested(mLastPinTab);
    });
